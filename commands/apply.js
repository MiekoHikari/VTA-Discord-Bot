...
module.exports = {
	...
	async execute(interaction) {
		...
		if (!userval) {
			...
			await interaction.editReply({ embeds: [createDialog], components: [createRow], ephemeral: true });
		}
		// If user has a profile in the database
		else {
			await interaction.deferReply({ ephemeral:true });
			...
			if (userval.Status == 'pending') {
				const validEmbed = new EmbedBuilder()
					.setColor(0xE0115F)
					.setTitle('An application is currently pending!')
					.setDescription('You cannot change your application until you are rejected!');

				await interaction.editReply({ embeds: [validEmbed], ephemeral: true });
			}

			if (userval.Status == 'Not Applied') {
				const yes = '**✅ Saved**';
				const no = '**❌ Pending**';
