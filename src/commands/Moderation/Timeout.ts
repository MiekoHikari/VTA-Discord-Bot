...
export class UserCommand extends Command {
	public override registerApplicationCommands(registry: Command.Registry) {
		registry.registerChatInputCommand((builder) =>
			builder //
				...
				.addStringOption(option => option.setName('duration').setDescription('How long the user shall be muted for. (eg. 5d 20h 3m 40s)').setRequired(true))
	...
	public override async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		...
		if (duration > (new Duration('5s')) || duration > (new Duration('28d'))) return interaction.reply({ content: 'Duration must be in range of 5s - 28d', ephemeral: true })
	...
	private async logAction(interaction: Command.ChatInputCommandInteraction, target: User, reason: string, duration: Duration) {
		...
		const embed = new EmbedBuilder()
			...
			.addFields({ name: 'Moderator', value: `<@${interaction.user.id}>`, inline: true }, { name: 'Target', value: `<@${target?.id}>`, inline: true }, {name: 'Duration', value: `${new DurationFormatter().format(duration.offset)}`}, { name: 'Reason', value: `${reason}`, inline: false})
			.setColor('Red');
